...
<Callout type="info">
**One command, full stack:** Running `zuro add auth` automatically installs database, validator, and error-handler if they're missing. No need to remember the order!
</Callout>
...
|--------|---------|--------------|
| **Core** | `zuro init` | Express, TypeScript, Helmet, CORS, Pino logger |
| **Database** | `zuro add database` | Drizzle ORM, PostgreSQL or MySQL |
| **Auth** | `zuro add auth` | Better-Auth, signup/login, sessions |
| **Validator** | `zuro add validator` | Zod middleware for request validation |
| **Error Handler** | `zuro add error-handler` | Custom error classes, consistent responses |
...
After `zuro init` + adding modules:
